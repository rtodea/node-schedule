...
Job.prototype.schedule = function(spec) {
  ...
  var previousJob = scheduledJobs[this.name];
  // TODO: why would you want anonymous jobs treated otherwise?
  if (previousJob && !isJobAnonymous(previousJob)) {
    previousJob.cancel();
  }
...
function isJobAnonymous(job) {
  return (new RegExp('<Anonymous ')).test(job.name);
}
